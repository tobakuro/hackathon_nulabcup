...
export async function getRepoDependencies(
    ...
    try {
        ...
        try {
            ...
            const [upsertedRepo] = await db.insert(repositories)
                .values({
                    owner,
                    name: repo,
                    fullName,
            ...
            // 既存のファイルを一度削除して洗い替え
            await db.delete(repositoryFiles).where(eq(repositoryFiles.repositoryId, repoId));
            ...
            if (filesToInsert.length > 0) {
                ...
                await db.insert(repositoryFiles).values(filesToInsert);
            }
        } catch (dbError) {
